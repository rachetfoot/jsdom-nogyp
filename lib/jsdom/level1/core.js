...
core.Node.prototype = {
  ...
  insertBefore :  function(/* Node */ newChild, /* Node*/ refChild) {
    ...
    if (this._ownerDocument !== newChild._ownerDocument) {
      // adopt the node when it's not in this document
      this._ownerDocument.adoptNode(newChild);
    } else {
  ...
  removeChild : function(/* Node */ oldChild){
    ...
    // The following shouldn't be necessary, but splice wouldn't work properly!
    var nodes = Array.prototype.slice.call(this._childNodes, 0);
	this._childNodes = new core.NodeList();
	Array.prototype.push.apply(this._childNodes,nodes.slice(0,oldChildIndex));
	Array.prototype.push.apply(this._childNodes,nodes.slice(oldChildIndex+1));
