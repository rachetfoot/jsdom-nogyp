...
exports.windowAugmentation = function(dom, options) {
  ...
  if (doc.addEventListener) {
    ...
    doc.addEventListener('error', function(ev) {
      if (ev.error) {
        text = "Error encountered: " + ev.error.message + " at \"" + ev.error.fileName + "\", line " + ev.error.lineNumber + 
			(ev.error.evalFile ? " while evaluating " + ev.error.evalFile : "") + "\n" + ev.error.stack;
      } else {
        text = 'Unknown error';
      };
	  print(text);
      window.raise('error',text);
    },true);
...
exports.createWindow = function(dom, options) {
  ...
  DOMWindow.prototype = {
    ...
    navigator: {
      userAgent: 'jsdom on '+rhinoVersion,
      appName: 'jsDom',
      platform: rhinoVersion,
      appVersion: rhinoVersion,
      noUI: true
    },
...
var getDefaultParser = exports.getDefaultParser = function () {
  if (defaultParser === null) {
	defaultParser = require('node-htmlparser');
  }
