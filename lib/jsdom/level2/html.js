...
core.resourceLoader = {
  ...
  download: function(url, cookie, cookieDomain, referrer, callback) {
    ...
    if (url.protocol === 'https:') {
      ...
      callback(new Error('Not possible in this version of jsdom'),null);
    } else {
  ...
  readFile: function(url, callback) {
    fs.readFile(url.replace(/^file:\/\//, "").replace(/^classpath:/, "").replace(/^\/([a-z]):\//i, '$1:/').replace(/%20/g, ' '), 'utf8', callback);
  }
};
